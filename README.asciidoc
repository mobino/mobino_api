...
+transaction.reference_number+:: An UNIQUE reference number that you can use to track
...
is present, your default account currency will be used)
| +api_key+             | your API key
| +reference_number+    | a reference number for that will be passed back to your server (Only Alphanumeric characters, +-+, and +_+ are allowed)
